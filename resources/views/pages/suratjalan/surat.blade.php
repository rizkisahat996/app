                  ...
                  @else
                      @foreach ($hasil as $item)
                          <tr>
                              ...
                              <td>{{ $item->kodejalan }}</td>
                              <td>{{ $item->nama }}, {{ $item->perusahaan }}</td>
                              <td>{{ date('d-m-Y', strtotime($item->created_at)) }}</td>
                              <td class="text-start ms-5 ps-5">{{ $item->nomor_polisi }}</td>
                              <td class="d-flex justify-content-center gap-3"> 
                                <a target="_blank"
                                      href="/preview-surat/{{ $item->id }}">
                                  ...
                                  @if (auth()->user()->jabatan == 'superadmin')
                                            ...
                                            @component('components.modal')
                                                ...
                                                @slot('isi')
                                                    <div class="py-4">
                                                        <h3 style="text-align: center">Apakah Anda yakin ingin menghapus Surat ini?</h6>
                                                        <div class="d-flex justify-content-center mt-4">
                                                            <form action="suratjalan/{{ $item->id }}" class="btn btn-danger" method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button style="appearance: none" type="submit" class="bg-danger">
                                                                    <i class="ti ti-trash"></i>
                                                                    <span>Hapus</span>
                                                                </button>
                                                            </form>
